CreateClientConVar("cl_3Dmodel_crosshair_size", 10)
	...
	if plr:Alive() then
	    ...
	    if headmatrix then
	        ...
	        if headmatpos == headmatpos then
	    	    local view = {}
	    	    view.origin = (headmatpos + (headang:Up() * 6.6) + (headang:Forward() * 6.5)) + (plr:GetVelocity() * 0.005)
	    	    view.angles = headang
	    	    view.fov = fov
	    ...
	    if IsValid(ragdoll) then
	    	...
	    	if headmatrix then
	            ...
	            if headmatpos == headmatpos then
	    	        local view = {}
	    	        view.origin = (headmatpos + (headang:Up() * 5) + (headang:Forward() * 6.5)) + (plr:GetVelocity() * 0.005)
	    	        view.angles = headang
	    	        view.fov = fov
	    	        view.drawviewer = true
