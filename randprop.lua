...
CreateClientConVar("cl_randprop_saypath", 0)

CreateClientConVar("cl_randprop_mount", 1)


local function AddToPropPool(path, tablechosen)
    local mount = GetConVar("cl_randprop_mount"):GetFloat()
    if mount > 0 then
...
local sayenabled = GetConVar("cl_randprop_saypath"):GetFloat()
