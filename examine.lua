CreateClientConVar("cl_examine_props", 0)
CreateClientConVar("cl_examine_ragdolls", 0)
CreateClientConVar("cl_examine_npcs", 0)
CreateClientConVar("cl_examine_players", 1)
CreateClientConVar("cl_examine_other", 0)
...
if hook.GetTable()["HUDPaint"]["examine"] then
	...
	local enableprops = GetConVar("cl_examine_props"):GetFloat()
	local enableragdolls = GetConVar("cl_examine_ragdolls"):GetFloat()
	local enablenpcs = GetConVar("cl_examine_npcs"):GetFloat()
	local enableplayers = GetConVar("cl_examine_players"):GetFloat()
	local enableother = GetConVar("cl_examine_other"):GetFloat()


	if enableother > 0 then
	    for _, other in ipairs(ents.GetAll()) do
		if IsValid(other) then
		    if !other:IsPlayer() and !other:IsNPC() and !other:IsRagdoll() and other:GetClass() ~= "prop_physics" then
	    	    	local otherpos = other:WorldSpaceCenter()
	    	    	local othermarker = otherpos:ToScreen()
		    	draw.SimpleText("Other Entity" .. " " .. other:GetClass(), "DefaultFixedDropShadow", othermarker.x, othermarker.y, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		    end
	    	end
	    end
	end

	if enableprops > 0 then
		...
		if player ~= ent then
			...
			if player:Alive() then
			    ...
			    phtextwhite = 0
	    	    	...
	    	    	draw.RoundedBox(10, marker.x - surface.GetTextSize(player:Nick()) / 2 - 5, marker.y - 15, (surface.GetTextSize(player:Nick()) + surface.GetTextSize(player:EntIndex()) + surface.GetTextSize(player:EntIndex())) + 10, 30, Color(phred, phgreen, 0))
	    	    	...
	    	    	draw.SimpleText(player:EntIndex(), "CloseCaption_Bold", marker.x + (surface.GetTextSize(player:Nick()) / 2 + surface.GetTextSize(player:EntIndex())), marker.y, Color(0, 0, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		    end
		end
	    end
	end
